export const APP_DOMAIN = "lingtext.de";
export const APP_ORIGIN = `https://${APP_DOMAIN}`;

export const SOURCE_LANGUAGE = "en";
export const TARGET_LANGUAGE = "de";
export const TARGET_LOCALE = "de-DE";

export const WEB_DB_NAME = "lingtext-de.sqlite3";
export const FILE_HANDLES_DB_NAME = "lingtext-de-handles";

export const TRANSLATOR_STORAGE_KEY = "lingtext-de.translator";
export const SEED_STORAGE_KEY = "lingtext-de:seeded:v1";
export const TEXT_WELCOME_MODAL_KEY = "lingtext-de_texts_welcome_modal_v1";
